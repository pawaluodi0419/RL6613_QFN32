...
u8 _by_Pattern3_dut2()
{
	...
	case 0x0000:
	{
		...
		{
			Buff_dut2_XGPIO_0[0] = 0x24;                            //REG0005 ouput value[7:0]
			...
			xil_printf("dut2.pattern3_gpio_control_completed!\r\n");
			msdelay(10);
		}
	...
	case 0x0001:
	{
		...
		dut2.g_pattern_smbus_control_buf[5] = 0x03;
	...
	case 0x0002:
	{
		...
		else
		{
			...
			if(dut2.g_pattern_smbus_control_buf[10] == 0x83)
			{
				for(i=1; i<31; i++)
				{
					dut2.g_pattern_smbus_control_buf[i] = CLEAR_;
...
/*
	...
	case 0x0003:
	{
		...
		else
		{
			...
			vmon_1v_adc_code_high = (dut2.g_pattern_smbus_control_buf[10]<<2);
	...
	case 0x0004:
	{
		...
		{
			//xil_printf("dut2.g_pattern_smbus_control_buf[10] =%x\r\n", dut2.g_pattern_smbus_control_buf[10]);
			vmon_1v_adc_code_low = dut2.g_pattern_smbus_control_buf[10];
			vmon_1v_adc_code = (vmon_1v_adc_code_high | vmon_1v_adc_code_low) & 0x3ff;
			xil_printf("dut2 read_adc_count =%x   vmon_1v_adc_code =%x\r\n", read_adc_index, vmon_1v_adc_code);
			...
			if(read_adc_count == 0)
			{
				...
				dut2.g_uartPatternNum = (dut2.g_uartPatternNum++ & 0xff);
				result_output_for_v50(XPAR_AXI_GPIO_dut2_1_BASEADDR,dut2.g_uartPatternNum);
				xil_printf("dut2 vmon_1v_adc_code_average =%x\r\n", vmon_1v_adc_code_average);
				xil_printf("dut2.pattern2_pass!\r\n\r\n");
			}
			else
			{
				for(i=1; i<31; i++)
...
*/
	...
	{
		xil_printf("dut2.pattern3_fail! fail_step = %x\r\n",dut2.g_pattern_step);
