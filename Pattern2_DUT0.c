...
u8 _by_Pattern2_dut0()
{
	...
	{
	//GPIO8接24M OSC, S0_FPGA_RELAY001(XGPIO5)设定输出high
	case 0x0000:
	{
		if(dut0.g_dut_pattern_status_buf[7] == 0x00)
		{
			...
			xil_printf("dut0.pattern2_gpio_control_completed!\r\n");
			msdelay(10);
		}
	...
	case 0x0001:
	{
		...
		dut0.g_pattern_smbus_control_buf[5] = 0x02;
	...
	case 0x0002:
	{
		...
		else
		{
			...
			//msdelay(10);
			if(dut0.g_pattern_smbus_control_buf[10] == 0x82)
			{
				for(i=1; i<31; i++)
				{
					dut0.g_pattern_smbus_control_buf[i] = CLEAR_;
				...
				//dut0.g_ack_bit_reread_timer = 10;
	...
	{
		xil_printf("dut0.pattern2_fail! fail_step = %x\r\n",dut0.g_pattern_step);
