...
u8 smbus0_irq_handle(u8 smbus_control_buf[])
{
	...
	u8  smbus0_process_state = dut0.g_pattern_smbus_control_buf[0];
	u8  smbus0_cmd_code = smbus_control_buf[1];
	u8  smbus_get_pinstatus_result = dut0.g_smbus_status_buf[2];
	...
	//////////xil_printf("smbus0_timer_initial\r\n");
	//////////xil_printf("dut0_smbus_style = %x\r\n", dut0.g_smbus_style);
	//////////xil_printf("dut0_smbus_timer = %x\r\n", dut0.g_smbus_timer);


	//smbus_write//
	if(((smbus0_cmd_code & 0x30) == 0x10) | (smbus0_cmd_code == 0x04))
	{
		...
		case smbus_road_waiting:
		{
			...
			smbus_master_write_blockwrite(SMBUS_DEV0_ADDR, smbus_writecmd_buf, smbus_writecmd_count);
	...
	//smbus_read//
	if(((smbus0_cmd_code & 0x30) == 0x30) | (smbus0_cmd_code == 0x24))
	{
		switch(smbus0_process_state)
		{
		case smbus_road_waiting:
		{
			//xil_printf("dut0_smbus_road_waiting_doing_read_case0000\r\n");
			dut0.g_smbus_status_buf[0] = CLEAR_;
			dut0.g_smbus_status_buf[1] = CLEAR_;
			dut0.g_smbus_status_buf[2] = CLEAR_;
			...
			isr_reg = XIic_ReadReg(SMBUS_DEV0_ADDR,  XIIC_IISR_OFFSET);	//interrupt status register
		...
		case smbus_road_doing:
		{
			...
			case 0x00:
			{
				...
				//xil_printf("\r\n dut0_smbus_road_doing_doing_read_case00\r\n");
			...
			{
		        //xil_printf("\r\n doing_read_case01\r\n");
				statusreg = XIic_ReadReg(SMBUS_DEV0_ADDR,  XIIC_SR_REG_OFFSET);	//status register bit2
				if((statusreg & XIIC_SR_BUS_BUSY_MASK) == 1)
				{
					dut0.g_pattern_smbus_control_buf[0] = smbus_master_communication_fail;
					return 0;
			...
			case 0x03:
			{
				...
				if((statusreg & XIIC_SR_BUS_BUSY_MASK) == 0x01)
				{
					...
					dut0.g_smbus_status_buf[0]++;
					dut0.g_smbus_status_buf[1] = smbus_master_read_before_readblock_timer;
					dut0.g_smbus_style++;
					dut0.g_smbus_timer = smbus_master_read_before_readblock_timer;
				}
				else if((statusreg & XIIC_SR_BUS_BUSY_MASK) == 0)
				{
					dut0.g_pattern_smbus_control_buf[0] = smbus_road_doing;
			...
			case 0x04:
			{
				...
				smbus_master_read_blockread_dut0(SMBUS_DEV0_ADDR, smbus_readcmd_count);
				//xil_printf("dut0_smbus_read_datacount = %x\r\n", smbus_control_buf[4]);
				dut0.g_smbus_status_buf[0]++;
				dut0.g_smbus_status_buf[1] = smbus_master_write_pinstatus_timer;
				dut0.g_smbus_style++;
				dut0.g_smbus_timer = smbus_master_read_after_readblock_timer;
				//xil_printf("dut0.g_smbus_status_buf[2] = %x\r\n", dut0.g_smbus_status_buf[2]);
				break;
			}
			...
			case 0x05:
			{
				...
				if(dut0.g_smbus_status_buf[2] == 0x01)
				{
					if((statusreg & XIIC_SR_BUS_BUSY_MASK) == 0x01)
					{
						dut0.g_pattern_smbus_control_buf[0] = smbus_master_communication_fail;
	...
	if((smbus0_cmd_code == 0x02) | (smbus0_cmd_code == 0x03))
	{
		switch(smbus0_process_state)
		{
		case smbus_road_waiting:
