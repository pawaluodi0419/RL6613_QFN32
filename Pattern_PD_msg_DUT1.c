...
u8 _by_Pattern_PD_msg_dut1()
{
	...
	{
	//reset to flash前需满足进FT mode条件：XGPIO13设定输出high, XGPIO5设定输出low
	case 0x0000:
	{
	...
	//check GPIO8为high开始PD msg test：XGPIO5设定输出high
	//U2_MUX test relay control：XGPIO25设定输出low
	//DP正插test relay control：XGPIO27设定输出low
	case 0x0002:
	{
		if(dut1.g_dut_pattern_status_buf[7] == 0x00)
		{
			Buff_dut1_XGPIO_0[0] = 0x21;                            //REG0005 ouput value[7:0]
			Buff_dut1_XGPIO_0[1] = 0xC2;							//REG0006
			Buff_dut1_XGPIO_0[2] = 0x60;							//REG0007 output value[15:8]
			Buff_dut1_XGPIO_0[3] = 0x9F;							//REG0008
			Buff_dut1_XGPIO_0[4] = 0x00|(dut1.g_uartPatternNum); 	//REG0009 output value[23:16]
	...
	case 0x0005:
	{
		...
		dut1.g_pattern_smbus_control_buf[4] = 0x01;
		dut1.g_pattern_smbus_control_buf[5] = 0x00;
		dut1.g_pattern_smbus_control_buf[6] = 0x08;
