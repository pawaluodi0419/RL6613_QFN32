...
u8 smbus3_irq_handle(u8 smbus_control_buf[])
{
	...
	u8  smbus3_process_state = dut3.g_pattern_smbus_control_buf[0];
	u8  smbus3_cmd_code = smbus_control_buf[1];
	u8  smbus_get_pinstatus_result = dut3.g_smbus_status_buf[2];

	//////////xil_printf("smbus_get_pinstatus_result = %d\r\n", smbus_get_pinstatus_result);

	//dut3.g_smbus_style = dut3.g_smbus_status_buf[0];
	//dut3.g_smbus_timer = dut3.g_smbus_status_buf[1];

	//////////xil_printf("smbus3_timer_initial\r\n");
	//////////xil_printf("dut3_smbus_style = %x\r\n", dut3.g_smbus_style);
	//////////xil_printf("dut3_smbus_timer = %x\r\n", dut3.g_smbus_timer);


	//smbus_write//
	if(((smbus3_cmd_code & 0x30) == 0x10) | (smbus3_cmd_code == 0x04))
	{
		switch(smbus3_process_state)
		{
		case smbus_road_waiting:
		{
			//xil_printf("\r\n smbus_road_waiting_doing_write_case00_\r\n");
			//smbus_write_step1_writeblock
			dut3.g_smbus_status_buf[0] = CLEAR_;
			dut3.g_smbus_status_buf[1] = CLEAR_;
			dut3.g_smbus_status_buf[2] = CLEAR_;
			...
			isr_reg = XIic_ReadReg(SMBUS_DEV3_ADDR,  XIIC_IISR_OFFSET);
			if((isr_reg &0x02) != 0)
			{
			...
			dut3.g_smbus_status_buf[1] = smbus_master_write_blockwrite_timer;
			dut3.g_smbus_style++;
			dut3.g_smbus_timer = smbus_master_write_blockwrite_timer;
		...
		{
			if(dut3.g_smbus_timer != 0)
			{
				if((dut3.g_axiTmr2Cnt&0x7f)==0x64)
				{
			...
			else
			{
				...
				{
					//xil_printf("\r\n doing_write_case01\r\n");
					statusreg = XIic_ReadReg(SMBUS_DEV3_ADDR,  XIIC_SR_REG_OFFSET);
					if((statusreg & XIIC_SR_BUS_BUSY_MASK) == 1)
					{
						dut3.g_pattern_smbus_control_buf[0] = smbus_master_communication_fail;
						return 0;
					}
					else
					{
						dut3.g_smbus_status_buf[0]++;
						dut3.g_smbus_status_buf[1] = smbus_master_write_before_pinstatus_timer;
						dut3.g_smbus_style++;
				...
				case 0x02:
				{
					...
					//dut3.g_smbus_style++;
					dut3.g_smbus_timer = smbus_master_write_pinstatus_timer;
					...
					case 0x00:
					{
						if(dut3.g_smbus_retry_timer >0)
						{
							dut3.g_smbus_retry_timer--;
						}
						else
						{
							dut3.g_result_fail=0x01;
							...
							dut3.g_smbus_status_buf[1] = clear_;
							dut3.g_smbus_status_buf[2] = clear_;
							dut3.g_smbus_style = clear_;
							dut3.g_smbus_timer = clear_;
						}
						break;
					}
					...
					{
						dut3.g_smbus_status_buf[0]++;
						dut3.g_smbus_status_buf[2] = 0x01;
						dut3.g_smbus_style++;
						break;
					}
				...
				case 0x03:
				{
					...
					//xil_printf("smbus_get_pinstatus_result = %d\r\n", smbus_get_pinstatus_result);
					//xil_printf("dut3_statusreg = %d\r\n", statusreg);

					if((statusreg & XIIC_SR_BUS_BUSY_MASK) == 0x01)
					{
						dut3.g_pattern_smbus_control_buf[0] = smbus_master_communication_fail;
						smbus3_process_state = smbus_master_communication_fail;
					...
					{
						dut3.g_pattern_smbus_control_buf[0] = smbus_master_communication_pass;
						smbus3_process_state = smbus_master_communication_pass;
	...
	//smbus_read//
	if(((smbus3_cmd_code & 0x30) == 0x30) | (smbus3_cmd_code == 0x24))
	{
		switch(smbus3_process_state)
		{
		case smbus_road_waiting:
		{
			//xil_printf("dut3_smbus_road_waiting_doing_read_case0000\r\n");
			dut3.g_smbus_status_buf[0] = CLEAR_;
			dut3.g_smbus_status_buf[1] = CLEAR_;
			dut3.g_smbus_status_buf[2] = CLEAR_;
		    ...
		    smbus_master_write_blockwrite(SMBUS_DEV3_ADDR, smbus_writecmd_buf, 5);
			isr_reg = XIic_ReadReg(SMBUS_DEV3_ADDR,  XIIC_IISR_OFFSET);	//interrupt status register
		...
		case smbus_road_doing:
		{
			...
			case 0x00:
			{
				...
				//xil_printf("\r\n dut3_smbus_road_doing_doing_read_case00\r\n");
			...
			{
		        //xil_printf("\r\n doing_read_case01\r\n");
				statusreg = XIic_ReadReg(SMBUS_DEV3_ADDR,  XIIC_SR_REG_OFFSET);	//status register bit2
				if((statusreg & XIIC_SR_BUS_BUSY_MASK) == 1)
				{
					dut3.g_pattern_smbus_control_buf[0] = smbus_master_communication_fail;
					return 0;
				...
				else
				{
					...
					dut3.g_smbus_retry_timer = 0x4e20;
					dut3.g_pattern_smbus_control_buf[0] = smbus_road_doing;
					smbus3_process_state = smbus_road_doing;
			...
			case 0x03:
			{
				...
				if((statusreg & XIIC_SR_BUS_BUSY_MASK) == 0x01)
				{
					...
					dut3.g_smbus_status_buf[0]++;
					dut3.g_smbus_status_buf[1] = smbus_master_read_before_readblock_timer;
					dut3.g_smbus_style++;
					dut3.g_smbus_timer = smbus_master_read_before_readblock_timer;
				}
				else if((statusreg & XIIC_SR_BUS_BUSY_MASK) == 0)
				{
					dut3.g_pattern_smbus_control_buf[0] = smbus_road_doing;
			...
			case 0x04:
			{
				...
				smbus_master_read_blockread_dut3(SMBUS_DEV3_ADDR, smbus_readcmd_count);
				//xil_printf("dut3_smbus_read_datacount = %x\r\n", smbus_control_buf[4]);
				dut3.g_smbus_status_buf[0]++;
				dut3.g_smbus_status_buf[1] = smbus_master_write_pinstatus_timer;
				dut3.g_smbus_style++;
				dut3.g_smbus_timer = smbus_master_read_after_readblock_timer;
				//xil_printf("dut3.g_smbus_status_buf[2] = %x\r\n", dut3.g_smbus_status_buf[2]);
				break;
			}
			...
			case 0x05:
			{
				...
				if(dut3.g_smbus_status_buf[2] == 0x01)
				{
					if((statusreg & XIIC_SR_BUS_BUSY_MASK) == 0x01)
					{
						dut3.g_pattern_smbus_control_buf[0] = smbus_master_communication_fail;
	...
	if((smbus3_cmd_code == 0x02) | (smbus3_cmd_code == 0x03))
	{
		switch(smbus3_process_state)
		{
		case smbus_road_waiting:
