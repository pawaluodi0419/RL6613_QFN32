...
u8 _by_Pattern5_dut0()
{
	...
	case 0x0000:
	{
		if(dut0.g_dut_pattern_status_buf[7] == 0x00)
		{
			...
			xil_printf("dut0.pattern5_gpio_control_completed!\r\n");
			msdelay(10);
		}
	...
	case 0x0001:
	{
		...
		dut0.g_pattern_smbus_control_buf[5] = 0x05;
	...
	case 0x0002:
	{
		...
		else
		{
			...
			if(dut0.g_pattern_smbus_control_buf[10] == 0x85)
			{
				for(i=1; i<31; i++)
				{
					dut0.g_pattern_smbus_control_buf[i] = CLEAR_;
	...
	{
		xil_printf("dut0.pattern5_fail! fail_step = %x\r\n",dut0.g_pattern_step);
