...
u8 _by_Pattern8_dut1()
{
	...
	case 0x0000:
	{
		if(dut1.g_dut_pattern_status_buf[7] == 0x00)
		{
			...
			xil_printf("dut1.pattern8_gpio_control_completed!\r\n");
			msdelay(10);
		}
	...
	//驱动DAC MCP4822 VOUTA(channel CC1)输出0.75V
	case 0x0001:
	{
		dut1.g_dac_data_buf[0] = 0x01;	//0x01: CC1 0x03: CC2	如果电压值超过2V, 需要写成0x05/0x07
		dut1.g_dac_data_buf[1] = 0xdc;	//Data L
		dut1.g_dac_data_buf[2] = 0x05;	//Data H
		dut1.g_dac_data_buf[3] = 0xa0;	//Data L
		dut1.g_dac_data_buf[4] = 0x05;	//Data H
		dut1.g_dac_data_buf[5] = 0x18;	//Data L
		dut1.g_dac_data_buf[6] = 0x06;	//Data H
	...
	//驱动DAC MCP4822 VOUTB(channel CC2)输出0.75V
	case 0x0002:
	{
	...
	case 0x0004:
	{
		...
		else
		{
			...
			if(dut1.g_pattern_smbus_control_buf[10] == 0x88)
			{
				for(i=1; i<31; i++)
				{
					dut1.g_pattern_smbus_control_buf[i] = CLEAR_;
	...
	{
		xil_printf("dut1.pattern8_fail! fail_step = %x\r\n",dut1.g_pattern_step);
