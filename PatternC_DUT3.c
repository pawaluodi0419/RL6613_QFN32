...
u8 _by_PatternC_dut3()
{
	...
	case 0x0000:
	{
		if(dut3.g_dut_pattern_status_buf[7] == 0x00)
		{
			...
			xil_printf("dut3.patternC_gpio_control_completed!\r\n");
			msdelay(10);
		}
	...
	case 0x0001:
	{
		...
		dut3.g_pattern_smbus_control_buf[5] = 0x0c;
	...
	case 0x0002:
	{
		...
		else
		{
			...
			if(dut3.g_pattern_smbus_control_buf[10] == 0x8C)
			{
				for(i=1; i<31; i++)
				{
					dut3.g_pattern_smbus_control_buf[i] = CLEAR_;
	...
	{
		xil_printf("dut3.patternC_fail! fail_step = %x\r\n",dut3.g_pattern_step);
