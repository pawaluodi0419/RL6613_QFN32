...
u8 _by_PatternC_dut2()
{
	...
	{
	//VCON至CC1/CC2 OCP功能测试???
	//XGPIO0设定输出high, XGPIO24设定输出low, XGPIO23设定输出high, XGPIO22设定输出high, XGPIO21设定输出low
	case 0x0000:
	{
		if(dut2.g_dut_pattern_status_buf[7] == 0x00)
		{
			...
			xil_printf("dut2.patternC_gpio_control_completed!\r\n");
			msdelay(10);
		}
	...
	case 0x0001:
	{
		...
		dut2.g_pattern_smbus_control_buf[5] = 0x0c;
	...
	case 0x0002:
	{
		...
		else
		{
			...
			if(dut2.g_pattern_smbus_control_buf[10] == 0x8C)
			{
				for(i=1; i<31; i++)
				{
					dut2.g_pattern_smbus_control_buf[i] = CLEAR_;
	...
	{
		xil_printf("dut2.patternC_fail! fail_step = %x\r\n",dut2.g_pattern_step);
