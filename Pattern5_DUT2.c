...
u8 _by_Pattern5_dut2()
{
	...
	{
	//1.VMON切换到ADC 0V: XGPIO4设定输出low, XGPIO14设定输出high
	//2.BMC TX relay control: XGPIO0设定输出low, XGPIO2设定输出high, XGPIO3设定输出low
	case 0x0000:
	{
		if(dut2.g_dut_pattern_status_buf[7] == 0x00)
		{
			...
			xil_printf("dut2.pattern5_gpio_control_completed!\r\n");
			msdelay(10);
		}
	...
	case 0x0001:
	{
		...
		dut2.g_pattern_smbus_control_buf[5] = 0x05;
	...
	case 0x0002:
	{
		...
		else
		{
			...
			if(dut2.g_pattern_smbus_control_buf[10] == 0x85)
			{
				for(i=1; i<31; i++)
				{
					dut2.g_pattern_smbus_control_buf[i] = CLEAR_;
	...
	{
		xil_printf("dut2.pattern5_fail! fail_step = %x\r\n",dut2.g_pattern_step);
