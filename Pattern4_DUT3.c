...
u8 _by_Pattern4_dut3()
{
	...
	case 0x0000:
	{
		...
		{
			Buff_dut3_XGPIO_0[0] = 0x3C;                            //REG0005 ouput value[7:0]
			...
			xil_printf("dut3.pattern4_gpio_control_completed!\r\n");
			msdelay(10);
		}
	...
	case 0x0001:
	{
		...
		dut3.g_pattern_smbus_control_buf[5] = 0x04;
	...
	case 0x0002:
	{
		...
		else
		{
			...
			if(dut3.g_pattern_smbus_control_buf[10] == 0x84)
			{
				for(i=1; i<31; i++)
				{
					dut3.g_pattern_smbus_control_buf[i] = CLEAR_;
...
/*
	...
	case 0x0003:
	{
		...
		else
		{
			...
			vmon_0p25v_adc_code_high = (dut3.g_pattern_smbus_control_buf[10]<<2);
	...
	case 0x0004:
	{
		...
		{
			//xil_printf("dut3.g_pattern_smbus_control_buf[10] =%x\r\n", dut3.g_pattern_smbus_control_buf[10]);
			vmon_0p25v_adc_code_low = dut3.g_pattern_smbus_control_buf[10];
			vmon_0p25v_adc_code = (vmon_0p25v_adc_code_high | vmon_0p25v_adc_code_low) & 0x3ff;
			xil_printf("dut3 read_adc_count =%x   vmon_0p25v_adc_code =%x\r\n", read_adc_index, vmon_0p25v_adc_code);
			...
			if(read_adc_count == 0)
			{
				...
				dut3.g_uartPatternNum = (dut3.g_uartPatternNum++ & 0xff);
				result_output_for_v50(XPAR_AXI_GPIO_dut3_1_BASEADDR,dut3.g_uartPatternNum);
				xil_printf("dut3 vmon_0p25v_adc_code_average =%x\r\n", vmon_0p25v_adc_code_average);
...
*/
	...
	{
		xil_printf("dut3.pattern4_fail! fail_step = %x\r\n",dut3.g_pattern_step);
